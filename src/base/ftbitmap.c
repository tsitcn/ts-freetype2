...
#define FT_TS_COMPONENT  bitmap
                           ...
                           FT_TS_UInt     ypixels )
  {
    ...
    width  = bitmap->width;
    height = bitmap->rows;
    pitch  = (unsigned int)FT_TS_ABS( bitmap->pitch );
    ...
    if ( FT_TS_QALLOC_MULT( buffer, bitmap->rows + ypixels, new_pitch ) )
      return error;
 ...
 */
  ...
  {
    /* check the value ? */
    return FT_TS_Bitmap_WeightXY(library, bitmap, strength, strength);
  }
  ...
  FT_TS_Bitmap_EmboldenXY( FT_TS_Library  library,
                      ...
                      FT_TS_Pos      yStrength )
  {
    return FT_TS_Bitmap_WeightXY(library, bitmap, xStrength, xStrength);
  }
  
  FT_TS_EXPORT( FT_TS_Error )
  FT_TS_Bitmap_WeightXY( FT_TS_Library  library,
                      FT_TS_Bitmap*  bitmap,
                      FT_TS_Pos      xStrength,
                      FT_TS_Pos      yStrength )
  {
 ...
 */
    ...
    if ( ( ( FT_TS_PIX_ROUND( xStrength ) >> 6 ) > FT_TS_INT_MAX ) ||
         ( ( FT_TS_PIX_ROUND( yStrength ) >> 6 ) > FT_TS_INT_MAX ) )
      return FT_TS_THROW( Invalid_Argument );

    xstr = (FT_TS_Int)FT_TS_PIX_ROUND( xStrength ) >> 6;
    ystr = (FT_TS_Int)FT_TS_PIX_ROUND( yStrength ) >> 6;
    ...
    error = ft_bitmap_assure_buffer( library->memory, bitmap,
                                     (FT_TS_UInt)xstr, (FT_TS_UInt)ystr );
    if ( error )
      return error;
    ...
    {
      pitch = -pitch;
      p = bitmap->buffer + (FT_TS_UInt)pitch * ( bitmap->rows - 1 );
    }
    ...
    for ( y = 0; y < bitmap->rows; y++ )
    {
      ...
      for ( x = pitch - 1; x >= 0; x-- )
      {
        ...
        for ( i = 1; i <= xstr; i++ )
        {
          if ( bitmap->pixel_mode == FT_TS_PIXEL_MODE_MONO )
          {
            p[x] |= tmp >> i;
    ...
    bitmap->width += (FT_TS_UInt)xstr;
    bitmap->rows += (FT_TS_UInt)ystr;
